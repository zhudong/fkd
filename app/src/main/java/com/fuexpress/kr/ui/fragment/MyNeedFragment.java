...
public class MyNeedFragment extends BaseFragment<MainActivity> {
    ...
    public void setInstorageCount(int count) {
        if (count > 0) {
            mToInstorageCountTV.setVisibility(View.VISIBLE);
            mToInstorageCountTV.setText(count + "");
        } else {
            mToInstorageCountTV.setVisibility(View.GONE);
        }
    }
    ...
    public void onEventMainThread(BusEvent event) {
        ...
        if (event.getType() == BusEvent.GO_MY_NEED) {
            if (rbgMyNeedFragment.getCheckedRadioButtonId() == R.id.rb_to_pay) {
                ToPayFragment fragment = (ToPayFragment) mFragmentManager.findFragmentByTag(TAB_TO_PAY);
                if (fragment != null)
                    fragment.reloade();
            } else {
                rbgMyNeedFragment.check(R.id.rb_to_pay);
