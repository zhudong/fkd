...
public interface HelpSignedContract {
    ...
    interface ItemView extends BaseViewGeneric<NewPresenter> {//这里是有关编辑或者添加的视图层需要实现的方法
        ...
        void showWareHouseChoose(boolean isEmpty);//显示仓库的选择界面
        ...
        void showMaterialInfoByBean(@NonNull MaterialsBean materialsBean);//显示材质信息

        void showCategoryInfoByBean(@NonNull CategoryBean categoryBean);//显示分类信息
    ...
    interface NewPresenter extends BasePresenterRx {//这里是有关逻辑层需要实现的一些操作接口汇总
        ...
        CategoryBean getDataCateoryBean();//获得数据的分类Bean对象

        void setDataCategoryBean(CategoryBean categoryBean);//设置分类数据的Bean对象

        void setDataMaterialBean(MaterialsBean materialBean);//设置材质数据的Bean对象

        MaterialsBean getDataMaterialBean();//获得数据的材质Bean对象
        ...
        void getWareHouseDataRemote();//从服务器中获取仓库列表
