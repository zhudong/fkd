...
public class AddressManagerAdapter extends BaseAdapter implements View.OnClickListener {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        //viewHolder.tv_in_am_item_address.setText(customerAddress.getStreet() + "," + AssetFileManager.getInstance().readFilePlus(customerAddress.getRegion(), AssetFileManager.ADDRESS_TYPE));
        viewHolder.tv_in_am_item_address.setText(customerAddress.getStreet() + "," + fullRegionName);
    ...
    public void showDialog(final int index) {
        CsAddress.CustomerAddress customerAddress = mData.get(index);
        boolean isDefault = customerAddress.getIsDefault();
        if (isDefault) {
            /*AddressManagerActivity context = (AddressManagerActivity) mContext;
            context.showProgressDiaLog(SweetAlertDialog.ERROR_TYPE, mContext.getString(R.string.default_not_delete));
            context.dissMissProgressDiaLog(600);*/
            CustomToast.makeText(mContext, mContext.getString(R.string.default_not_delete), Toast.LENGTH_SHORT).show();
        } else {
    ...
    private void switchIsDefault(int newDefault) {
        ...
        /*AddressManagerActivity addressManagerActivity = (AddressManagerActivity) mContext;
        addressManagerActivity.showProgressDiaLog(SweetAlertDialog.PROGRESS_TYPE, "加载中");*/
        //AddressManager.getInstance().setAddressDefault(switchAddressBuilder(newDefault));
        AddressManager.getInstance().setAddressDefault(switchAddressBuilder(newDefault), mContext.getClass().getName());
    }
    ...
    public void reSetDataList(List<CsAddress.CustomerAddress> dataList) {
        mData = dataList;
    }
