...
public class AdapterForHsSmall extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        if (mProductDataLists.size() < 1) {
            ...
            Glide.with(mContext).load(productDataList.getCoverImagePath()).fitCenter().into(viewHolder.iv_in_item_hs);
            ...
            String price = productDataList.getPrice();
            if (!price.contains(","))
                price = UIUtils.getCurrency(SysApplication.getContext()
                        , productDataList.getCurrency_Code(), Float.valueOf(productDataList.getPrice()));
            else
                price = UIUtils.getCurrency(mContext, productDataList.getCurrency_Code()) + price;
            viewHolder.tv_hs_item_price.setText(price);
