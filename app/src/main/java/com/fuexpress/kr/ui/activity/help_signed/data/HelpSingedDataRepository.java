...
public class HelpSingedDataRepository implements HelpSignedDataSource {
    ...
    public void setHelpSignedRemoteDataSource(@NonNull HelpSignedRemoteDataSource helpSignedRemoteDataSource) {
        ...
        HelpMeSingleBean wantAppendBean = mHelpSignedRemoteDataSource.getHelpMeSingleBean();
        /*if (mHelpSingleBeanList.size() > 0) {
            for (HelpMeSingleBean helpMeSingleBean : mHelpSingleBeanList) {
                if (helpMeSingleBean.getSalesrequireid() != 0)
                    if (helpMeSingleBean.getSalesrequireid() != wantAppendBean.getSalesrequireid())
                        mHelpSingleBeanList.add(wantAppendBean);
            }
        } else*/
            mHelpSingleBeanList.add(wantAppendBean);
    }
    ...
    public void reMoveHelpSingleBean(int index) {//删除
        if (mHelpSingleBeanList != null) {
            if(mHelpSingleBeanList.size()>index){
