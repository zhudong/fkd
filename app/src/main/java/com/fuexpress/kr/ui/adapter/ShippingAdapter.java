...
public class ShippingAdapter extends SimpleBaseAdapter<CsOrder.SalesOrderShipping> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(split.length>0){
            wareHouseName = split[0];
        }
//        holder.warehouseTv.setText(item.getWarehouseName());
        holder.warehouseTv.setText(wareHouseName);
        String shippingMethod = item.getShippingMethod().replace(wareHouseName, "").trim();
        holder.methodTv.setText(shippingMethod);
