...
public class UserManager {
    ...
    public void getUserDetailInfo() {
        ...
        builder.addFields(CsUser.UserInfoFieldType.USER_INFO_FIELD_MEMBER_GROUP_VALUE);
        /*CsBase.BaseUserRequest.Builder baseUserRequest = CsBase.BaseUserRequest.newBuilder();
        baseUserRequest.setMid(0);
        baseUserRequest.setUin(AccountManager.getInstance().mUin);
        baseUserRequest.setTicket("");*/
    ...
    private void setUserDetailInfo(List<CsUser.UserInfoField> fieldsList) {
        for (int x = 0; x < fieldsList.size(); x++) {
            ...
            } else if (CsUser.UserInfoFieldType.USER_INFO_FIELD_MEMBER_GROUP_VALUE == fieldType) {
                mMemberGroup = Integer.valueOf(userInfoField.getFieldValue());
            }
        }
    ...
    public void getUserSimpleInfo(int uin, final RequestNetListener listener) {
        ...
        builder.setAuthor(uin);
        builder.setBaseuser(AccountManager.getInstance().getBaseUserRequest());
        builder.setLocalecode(AccountManager.getInstance().getLocaleCode()).setType(1);
    ...
    public void getMemberGroupNameRequest(int memberGroupID) {
        CsUser.GetMemberGroupNameRequest.Builder builder = CsUser.GetMemberGroupNameRequest.newBuilder();
        ...
        NetEngine.postRequest(builder, new INetEngineListener<CsUser.GetMemberGroupNameResponse>() {


            @Override
            public void onSuccess(CsUser.GetMemberGroupNameResponse response) {
                mGroupName = response.getMemberGroupName();
                EventBus.getDefault().post(new BusEvent(BusEvent.GET_MEMBER_NAME_CONPLETE, true));
            }

            @Override
            public void onFailure(int ret, String errMsg) {
                EventBus.getDefault().post(new BusEvent(BusEvent.GET_MEMBER_NAME_CONPLETE, false, ret + errMsg));
            }
        });
    }
    ...
    public BottomStateBean getBottomStateBean() {
        if (mBottomStateBean != null)
            return mBottomStateBean;
        else return new BottomStateBean();
    }
}
