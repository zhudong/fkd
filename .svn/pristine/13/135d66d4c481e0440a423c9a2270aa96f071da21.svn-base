...
public class PaymentManager implements AdyenCheckoutListener {
    ...
    public void aliPay(final String payInfo, OnAliPayListener onAliPayListener) {
    ...
    class MyHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (TextUtils.equals(resultStatus, "9000")) {
                onAliPayListener.onSuccess(resultStatus);
...
//                Toast.makeText(ConfirmPaymentActivity.this, "支付成功", Toast.LENGTH_SHORT).show();
                ...
                if (TextUtils.equals(resultStatus, "8000")) {
                    onAliPayListener.onFailure(resultStatus, mCtx.getString(R.string.alipay_confirming_msg));
                    //Toast.makeText(mCtx, "支付结果确认中", Toast.LENGTH_SHORT).show();

                } else if (TextUtils.equals(resultStatus, "6001")) {
                    //Toast.makeText(mCtx, "支付取消", Toast.LENGTH_SHORT).show();
                    onAliPayListener.onFailure(resultStatus, mCtx.getString(R.string.alipay_canceled_msg));

                } else {
                    // 其他值就可以判断为支付失败，包括用户主动取消支付，或者系统返回的错误
                    onAliPayListener.onFailure(resultStatus, mCtx.getString(R.string.alipay_failed_msg) + " errCode=" + resultStatus);
    ...
    public void wechatPay(String payInfo) {
        if (!CommonUtils.isWeixinAvilible(mCtx)) {
            ((BaseActivity) mCtx).closeLoading();
            CustomToast.makeText(mCtx, "未安装微信，请更换支付方式", Toast.LENGTH_SHORT).show();
            EventBus.getDefault().post(new BusEvent(BusEvent.PAY_MENT_RESULT, false, SysApplication.mCurrentRequestPayment));
            return;
        }
...
//        Toast.makeText(this, String.valueOf(isPaySupported), Toast.LENGTH_SHORT).show();
    ...
    public void adyenPay(String paymentString, String orderNumber, long orderId, int orderType, float amount, String currency, OnPayListener onPayListener) {

        getAdyenUserInfo(paymentString, Constants.URL_ADYEN_IS_SAVE_USER_INFO + "uin/" + AccountManager.getInstance().mUin, orderNumber, orderId, orderType, amount, currency, onPayListener);

    }

    public void daouPay(Context context, float needPay, int orderId,int type, OperaRequestListener listener) {
        DaoUPayActivity.IntentBuilder intentBuilder = DaoUPayActivity.IntentBuilder.build(context)
                .setAmount((int) needPay)
                .setOrderID(orderId)
                .setOrderType(type)
    ...
    @Override
    public void checkoutAuthorizedPayment(CheckoutResponse checkoutResponse, boolean isChecked) {
        ...
        try {
            ...
            jsonObject.put("recurring", isChecked ? 1 : 0);
    ...
    @Override
    public void checkoutFailedWithError(String s, int errorType) {
        if (onPayListener != null) {
            onPayListener.onError(s);
        }
    }

    @Override
    public void checkoutFastPay(AdyenUserInfoBean adyenInfo, CheckoutResponse checkoutResponse) {
        try {
            ...
            jsonObject.put("currency", checkoutResponse.getCurrency());
            jsonObject.put("paymentData", checkoutResponse.getPaymentData());
            jsonObject.put("recurringDetailReference", adyenInfo.details.get(0).RecurringDetail.recurringDetailReference);
            LogUtils.d(jsonObject.toString());
    ...
    public void getAdyenUserInfo(final String paymentString, final String url, final String orderNumber, long orderId, int orderType, final float amount, final String currency, OnPayListener onPayListener) {
//        Looper.prepare();
//        ((BaseActivity) mCtx).showLoading();
//        Looper.loop();
            ...
            public void run() {
                ...
                call.enqueue(new Callback() {
                    ...
                    public void onResponse(Call call, Response response) throws IOException {
                        ((BaseActivity) mCtx).closeLoading();
                        Gson gson = new Gson();
                        AdyenUserInfoBean adyenInfo = gson.fromJson(response.body().string(), AdyenUserInfoBean.class);
                        if (adyenInfo != null) {
                            try {
...
//                              checkoutRequest.setCheckoutAmount(Float.parseFloat(String.format(s, 1.25)));
                                ...
                                Intent intent = new MyPayActivity.PaymentActivityBuilder(checkoutRequest).build(PaymentManager.this, mCtx);
                                intent.putExtra("orderNumber", orderNumber);
                                intent.putExtra("paymentString", paymentString);
                                Bundle bundle = new Bundle();
                                bundle.putSerializable("userInfoBean", adyenInfo);
...
//            });
    ...
    public void doFastPayPost(final String json) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                OkHttpClient client = new OkHttpClient();
                ...
                try {
                    Request request = new Request.Builder().url(orderType == 0 ? Constants.CART_PAY_SAVED_URL : Constants.CARD_PAY_SAVED_URL).post(requestBody).build();
...
//                    Toast.makeText(mCtx, mCtx.getString(R.string.pay_error_msg), Toast.LENGTH_SHORT).show();
    ...
    public interface OnAliPayListener {
        void onSuccess(String resultStatus);

        void onFailure(String resultStatus, String errorMsg);
    }
...
//                                payString = mCtx.getString(R.string.String_krbank_pay);
                            }
...
//                                payString = mCtx.getString(R.string.String_weixin_pay);
                            }
                            if (TextUtils.equals(list.get(i).getPaycode(), Constants.GIFT_CARD_PAYMENT_DAOUPAY)) {
                                payType = Constants.PAYMENT_DAOUPAY;
//                                payString = list.get(i).getPaycode();
                            }
                            payCode = list.get(i).getPaycode();
                            payString = list.get(i).getPayname();
                            SPUtils.put(mCtx, AccountManager.getInstance().getCurrencyCode() + PaymentActivity.DEF_PAY_NAME, payString);
                            SPUtils.put(mCtx, AccountManager.getInstance().getCurrencyCode() + PaymentActivity.DEF_PAY_CODE, payCode);
                            SPUtils.put(mCtx, AccountManager.getInstance().getCurrencyCode(), payType);
                        }
                    }
                }
                onFKDPaymentListener.onResult(payType, payString, payCode);
            ...
            public void onFailure(int ret, final String errMsg) {
                ((Activity) mCtx).runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        CustomToast.makeText(mCtx, mCtx.getString(R.string.request_payment_error_msg) + " " + errMsg, Toast.LENGTH_SHORT).show();
                    }
                });
            }
        });
