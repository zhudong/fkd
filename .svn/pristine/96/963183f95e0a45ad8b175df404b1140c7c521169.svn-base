 ...
 * 5.2修改，添加国家。常用城市保存城市id，根据id显示城市。
 */
...
public class ChooseCityActivity extends BaseActivity {
    ...
    public void setCityList(Integer str){
        if(mCities==null){
            mCities=new ArrayList<>();
        }
        if(mCities.contains(str)){
            mCities.remove(new Integer(str));
        }
        mCities.add(0, str);
        saveCityList();
        setButton();
    ...
    private void setButton(){
        ...
        for(int i=0;i<mCities.size();i++){
            ...
            mButtons[i].setText(mCityMap.get(mCities.get(i)));
        ...
        if(TextUtils.isEmpty(str)){
            ...
            for(int i=0;i<citys.length;i++){
                ...
                if(!TextUtils.isEmpty(citys[i])&&!list.contains(citys[i])){
                    list.add(Integer.valueOf(citys[i]));
                }
            }
        }
        return list;
    ...
    private void checkCityAvailable(){
        int city=mResponse.getWarehouseRegionId();
        SPUtils.putString(this,ChooseCityActivity.F_RECENT_CITY_ID,city+"");
        mCities.remove(new Integer(city));
        mCities.add(0,city);
        for(int i=0;i<mCities.size();i++){
            int s=mCities.get(i);
            if(mCityMap.get(s)==null){
                mCities.remove(new Integer(s));
            }
        }
        ...
        for(int i=0;i<countryRegionInfos.size();i++){
            ...
            for(int j=0;j<regionInfos.size();j++){
                ...
                for(int k=0;k<regionCityInfos.size();k++){
                    ...
                    mCityMap.put(Integer.valueOf(info.getRegionId()),info.getRegionName());
                }
            }
        }
    }
    ...
    private void setCity(int i){
        ...
        int str=mCities.get(i);
        SPUtils.putString(ChooseCityActivity.this,ChooseCityActivity.F_RECENT_CITY_ID,str+"");
        SPUtils.putString(ChooseCityActivity.this,ChooseCityActivity.F_RECENT_CITY,mCityMap.get(str));
        if(mCityMap.get(str)!=null){
            saveWareHouse(str,null);
        }
        mCities.remove(new Integer(str));
        mCities.add(0,str);
        setButton();
