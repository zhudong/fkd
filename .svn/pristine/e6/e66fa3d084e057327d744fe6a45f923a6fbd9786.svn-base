...
public class OrderShowAdapter extends BaseAdapter {
    ...
    public OrderShowAdapter(@NonNull Context context, @NonNull List<CsBase.ItemImage> theDataList) {
        mContext = context;
    ...
    public OrderShowAdapter(@NonNull Context context, @NonNull ArrayList<String> theDataList) {
        mContext = context;
    ...
    public OrderShowAdapter(@NonNull Context context, @NonNull List<String> theDataList, int maxSize) {
        mContext = context;
        mImagePathList = theDataList;
        int width = UIUtils.getScreenWidthPixels((Activity) mContext) / 4 - 30;
        mLayoutParams = new LinearLayout.LayoutParams(width, width);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null || convertView.getTag(R.string.adapter_id_01) == null) {
        ...
        Glide.with(mContext).load(mImagePathList.get(position)).into(viewHolder.iv_in_item_help_gw);
