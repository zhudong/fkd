...
//        String fromWhere = getString(R.string.main_package_tab_string);
        ...
        sToSend = getString(R.string.String_parcel_goto_send);
    ...
    public void setTraceVisibility(boolean visibility, List<CsParcel.ParcelItemList> items) {
    ...
    private void toParcelSplit(CsParcel.Parcel parcel) {
...
//            final String instoreTime = "";
            ...
            if (outStore) {
                mTvOutStorageTime.setVisibility(View.VISIBLE);
                mTvOutStorageTime.setText(getString(R.string.package_out_house_time) + parcel.getOutstoreTime());
            }
...
//        setIdCardNum(address.getName(), address.getIdCard());
    ...
    public void showAddress(String address) {
        ...
        if (TextUtils.isEmpty(address.replaceAll(",", "").replaceAll("\n", ""))) {
            ...
            ssb.setSpan(new ForegroundColorSpan(getResources().getColor(R.color.black_parcel)), 0, address.indexOf("\n"), Spanned.SPAN_INCLUSIVE_INCLUSIVE);
            mTvPersonName.setText(ssb);
        }
    }
    ...
    public void showBalanceAndPayType(final String result) {
        UIUtils.postTaskSafely(new Runnable() {
            @Override
            public void run() {
                mTvAccountBalance.setVisibility(View.VISIBLE);
                mTvAccountBalance.setText(result);
            }
        });
...
//                    setIdCardNum(customerAddress.getIdCard());
        ...
        if (requestCode == 0 && resultCode == Constants.PAYMENT_REQUEST_CODE && data != null) {
            {
                mPresenter.setCouponsAndCalc(true);
            }
        }
...
//            mTvIdCardName.setText(idCardNumber);
    ...
    public void showParcleDialog(String message, final int type, final String cancelText) {
        final String split_text = getString(R.string.package_split_parcel);
        final String re_declaration = getString(R.string.package_re_declaration);
        String positiveString = getString(R.string.confirm);
        switch (type) {
            case 1:
                positiveString = split_text;
                break;
            case 2:
                positiveString = re_declaration;
                break;
        }
        ...
        builder.setPositiveButton(positiveString, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                switch (type) {
                    case 1:
                        toParcelSplit(mPresenter.getParcel());
                        break;
                    case 2:
                        insurance();
                        break;
