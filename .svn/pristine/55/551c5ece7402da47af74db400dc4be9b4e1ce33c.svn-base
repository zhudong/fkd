...
public class HelpHeadAdapter extends BaseAdapter {
    ...
    public HelpHeadAdapter(@NonNull Context context, @NonNull List<String> theDataList, int maxSize) {
        mContext = context;
        mImagePathList = theDataList;
        int width = UIUtils.getScreenWidthPixels((Activity) mContext) / 4 - 30;
        mLayoutParams = new LinearLayout.LayoutParams(width, width);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        Glide.with(mContext).load(mImagePathList.get(position)).fitCenter().into(viewHolder.iv_in_item_help_gw);
