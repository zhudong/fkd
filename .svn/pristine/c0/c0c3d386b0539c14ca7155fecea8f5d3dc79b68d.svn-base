...
public class AddressManager {
    ...
    public String getRegionTextByRAM(@NonNull String regionCode, @NonNull String type) {
        ...
        if (regionBean != null) {
            ...
            if (regionBean1 != null) {
                ...
                String cityString = "";
                if (!isProvince) {
                    RegionBean regionBean2 = mCodeCityMap.get(regionCode);
                    if (regionBean2 != null)
                        cityString = regionBean2.getRegionName();
                }
     ...
     * 设置该地址为默认
     */
    public void setAddressDefault(final CsAddress.CustomerAddress address, final String className) {
        ...
        NetEngine.postRequest(builder1, new INetEngineListener<CsAddress.EditCustomerAddressResponse>() {
            ...
            public void onSuccess(CsAddress.EditCustomerAddressResponse response) {
                ...
                AccountManager.getInstance().regionID = address.getRegionId();
                AccountManager.getInstance().countryCode = address.getCountryCode();
                EventBus.getDefault().post(new BusEvent(BusEvent.SET_ADDRESS_IS_DEFAULT_COMPETE, true, className));
            }

            @Override
            public void onFailure(int ret, String errMsg) {
                EventBus.getDefault().post(new BusEvent(BusEvent.SET_ADDRESS_IS_DEFAULT_COMPETE, false, className));
            }
        });
    }
